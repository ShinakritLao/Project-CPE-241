...
def get_productdata_dash(cur, salesperson, salesyear):
    ...
    cur.execute(f"SELECT s.salespersonid, sp.productid, sp.totalsales, sp.totalcost, sp.status, s.year FROM SalesProduct sp JOIN Sales s ON sp.salesid = s.salesid WHERE s.salespersonid = '{salesperson}' AND s.year = {salesyear};")
    result = cur.fetchall()

    # Convert the result to a pandas DataFrame
    productdata = pd.DataFrame(result, columns = ['SalesID', 'ProductID', 'Total Sales', 'Total Costs', 'Status', 'Year'])
    ...
    productdata['Year'] = productdata['Year'].astype(str)
