def get_details(cur, table, filter):
    ...
    if filter == 'Position':
        ...
        cur.execute(f"SELECT DISTINCT {filter} FROM {table} ORDER BY {filter} ASC")
        result = cur.fetchall()
...
def get_restore(cur):
    ...
    cur.execute("""
            ...
            (Action = 'Delete' AND LEFT(Location, 5) LIKE '%' || Original_Data || '%' AND Selected_Table != 'Users') ORDER BY ChangeID;
            """)
    result = cur.fetchall()
